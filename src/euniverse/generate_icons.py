...
def create_largephoto_icon():
    pixmap = QPixmap(28, 28)
    pixmap.fill(Qt.transparent)
    painter = QPainter(pixmap)
    painter.setRenderHint(QPainter.Antialiasing, False)
    
    # Fill entire 32x32 area with medium grey
    painter.setPen(Qt.NoPen)
    painter.setBrush(QBrush(Qt.lightGray))
    painter.drawRect(0, 0, 28, 28)
    
    # Draw 1 pixel wide red border
    painter.setPen(QPen(Qt.red, 1))
    painter.setBrush(Qt.NoBrush)
    painter.drawRect(0, 0, 27, 27)  # 31 because drawRect draws inside the rectangle
    
    # Draw black square 16x16 pixels
    # Center it: (32-16)/2 = 8 pixels from each edge
    painter.setPen(Qt.NoPen)
    painter.setBrush(QBrush(Qt.black))
    painter.drawRect(9, 9, 16, 16)
    
    painter.end()
    return QIcon(pixmap)

def create_mediumphoto_icon():
    pixmap = QPixmap(28, 28)
    pixmap.fill(Qt.transparent)
    painter = QPainter(pixmap)
    painter.setRenderHint(QPainter.Antialiasing, False)
    ...
    painter.setPen(QPen(Qt.red, 1))
    painter.setBrush(Qt.NoBrush)
    painter.drawRect(9, 9, 16, 16)  # 31 because drawRect draws inside the rectangle
    
    painter.end()
    return QIcon(pixmap)

def create_smallphoto_icon():
    pixmap = QPixmap(28, 28)
    pixmap.fill(Qt.transparent)
    painter = QPainter(pixmap)
    painter.setRenderHint(QPainter.Antialiasing, False)
    ...
    painter.setPen(QPen(Qt.red, 1))
    painter.setBrush(Qt.NoBrush)
    painter.drawRect(11, 11, 8, 8)  # 31 because drawRect draws inside the rectangle
